...
Harv_assump = read.csv("./_data_/Assumptions/none_antler_antler.csv",header = F)
...
Assumptions = list()
Assumptions$Harv = list(time = diag(period+1),age = Harv_assump) # tons of assumptions on vital rates
...
FYA_sex[9:11,4:6] = diag(3)
FYA_sex[1:3,1:3] = diag(3)
FYA_sex[3:8,3] = 1

Fec_assump = read.csv("./_data_/Assumptions/FYA_female.csv",header = F)
Fec_assump = as.matrix(Fec_assump)
Assumptions$Fec = list(time = diag(period),age = Fec_assump)
Assumptions$err  =list(time = matrix(1,1,period))
Surv_assump = read.csv("./_data_/Assumptions/FYA_sex.csv",header = F) |> as.matrix()
Assumptions$Surv = list(time = diag(period),age = Surv_assump)
#Assumptions$Surv = list(time = diag(period),age = diag(11))
...
Chicago_RES = ReCAP_sampler( Harv.data = as.matrix(Harv.data)
							...
							, n.iter = 1e2, burn.in = 5e1,thin.by = 50
...
save.image("./Main_analysis_Chicago/models_to_be_selected/2harv_4surv_3fec_equal/2harv_4surv_3fec_equal.RData")
